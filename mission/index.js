...
setTimeout(()=>{
    document.getElementById('ch-plugin-entry')?.classList.add('hide')}, 800)
// 미션 데이터 가져오기
    ...
    try {
        ...
        if (currentMissionData.verifyType === 'PHOTO') {
            ...
            if (result.data && result.data.points > 0) {
                const redirectUrl = `https://biocom.kr/arang-reward-modal?type=quiz&point=${result.data.points}`;
                console.log('포인트 획득 - 리다이렉트 URL:', redirectUrl);
                window.location.href = redirectUrl;
            } else {
                const redirectUrl = 'https://biocom.kr/arang-reward-modal?type=quiz';
