...
let selectedDate = null; // 추가: 선택된 날짜
let mealRecordId = null; // 추가: 식사 기록 ID
...
async function saveMealRecord(mealData) {
    try {
        ...
        console.log('요청 URL:', `${API_BASE_URL}/activity/${mealRecordId}?email=${MEMBER_UID}`);
        
        const response = await fetch(`${API_BASE_URL}/activity/${mealRecordId}?email=${MEMBER_UID}`, {
            method: 'PUT',
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(`식사 기록 수정에 실패했습니다. (${response.status})`);
        }
...
async function submitMeal() {
    ...
    try {
        ...
        alert('식사 기록 수정에 실패했습니다. 다시 시도해주세요.');
    } finally {
        // 버튼 상태 복원
...
// 날짜 포맷 함수 (YYYY-MM-DD → 4월 23일 (화) 등)
...
const load = async () =>{
    // 1. URL에서 date, mealType 파싱
    selectedDate = getQueryParam('date');
    const urlMealType = getQueryParam('mealType');
    if (urlMealType) selectedMealType = urlMealType;

    // 2. 날짜 표시
    if (selectedDate) {
        document.getElementById('date-display').textContent = formatDateDisplay(selectedDate);
        document.getElementById('meal-type-display').textContent = getMealTypeText(selectedMealType);
    }

    // 3. 끼니 선택 UI 반영
    document.querySelectorAll('.meal-option').forEach(opt => {
        if (opt.dataset.meal === selectedMealType) {
    ...
    uploadedFileId = null;
    mealRecordId = null; // 식사 기록 ID 초기화
    ...
    // 4. API로 데이터 조회 후 폼에 세팅
    if (selectedDate && selectedMealType) {
        try {
            const response = await fetch(`https://biocom.ai.kr/api/v1/activity/daily?email=${MEMBER_UID}&date=${selectedDate}`);
            const result = await response.json();
            if (result.success && Array.isArray(result.data)) {
                const mealData = result.data.find(item => item.type === 'DIET' && item.data.mealType === selectedMealType);
                if (mealData) {
                    // 식사 기록 ID 저장
                    mealRecordId = mealData.id;
                    
                    // [식사정보]
                    const mealInput = document.querySelector('.meal-input input');
                    if (mealData.data.isFasting) {
                        mealInput.value = '';
                        mealInput.placeholder = '공복 상태입니다';
                        mealInput.disabled = true;
                        document.querySelector('.toggle-switch').classList.add('active');
                    } else {
                        mealInput.value = mealData.data.menu ? mealData.data.menu.join(', ') : '';
                    ...
                    // [과민 음식 섭취 여부]
                    const sensitiveInput = document.querySelector('.sensitive-input input');
                    if (mealData.data.allergyFoods && mealData.data.allergyFoods.length > 0) {
                        // allergyFoods는 [{level, items}] 배열
                        const foods = mealData.data.allergyFoods.map(af => af.items).flat();
                        sensitiveInput.value = foods.length > 0 ? foods.join(', ') : '전부 섭취 안함';
                    } else {
                        sensitiveInput.value = '전부 섭취 안함';
                    }
                    // [식후 컨디션 메모]
                    const conditionInput = document.querySelector('.condition-input input');
                    conditionInput.value = mealData.data.memo || '';
                }
            }
        } catch (e) {
            console.error('식사 데이터 조회 실패:', e);
        }
    }
