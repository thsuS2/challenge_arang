...
function clearSurveyData() {
    try {
        ...
        // 오류 발생 시 대체 방법
        try {
            localStorage.setItem('surveyAnswers_af', '{}');
...
function loadCategory() {
    ...
    // 저장된 답변 불러오기
    const savedAnswers = JSON.parse(localStorage.getItem('surveyAnswers_af') || '{}');
    ...
    item.innerHTML = `
        ...
        <div class="rating-scale">
            ...
            <div class="scale-labels">
                <span class="scale-label negative">개선 되지 않았다</span>
                <span class="scale-label positive">매우 개선되었다</span>
            </div>
        </div>
    `;
...
async function nextStep() {
    ...
    localStorage.setItem('surveyAnswers_af', JSON.stringify(savedAnswers));

    // 다음 카테고리로 이동
    if (currentCategoryIndex < surveyData.categories.length - 1) {
        window.location.href = `https://biocom.kr/arang-analysis-af?category=${currentCategoryIndex + 2}`;
    } else {
        ...
        // 삭제 확인
        const checkData = localStorage.getItem('surveyAnswers_af');
        if (!checkData) {
...
function goBack() {
    if (currentCategoryIndex > 0) {
        window.location.href = `https://biocom.kr/arang-analysis-af?category=${currentCategoryIndex}`;
    } else {
        window.location.href = 'https://biocom.kr/arang-af-qeustion-intro';
    }
}
